...
const TASK_TYPE_LABELS = { exam: '試験', quiz: '小テスト', report: 'レポート', other: 'その他' };
  ...
  for (let d = 1; d <= last.getDate(); d++) {
    ...
    html += `<div class="cal-cell ${isToday ? 'cal-today' : ''}">
      ...
      ${tasks.slice(0, 2).map(t => {
        const col = PRIORITY_COLORS[t.priority] || '#888';
...
function renderTasks() {
      ...
      <div class="task-item-left">
        ...
        <div>
          ...
          <div class="task-meta">
            ...
            ・<span style="color:${PRIORITY_COLORS[t.priority]};font-weight:600;">${PRIORITY_LABELS[t.priority]}</span>
  ...
  document.querySelectorAll('.task-status-btn').forEach(btn => {
    btn.addEventListener('click', async e => {
      e.stopPropagation();
      const next = btn.dataset.status === 'pending' ? 'in_progress'
                 : btn.dataset.status === 'in_progress' ? 'done' : 'pending';
      try {
        await api(`/tasks/${btn.dataset.id}/status`, { method: 'PATCH', body: JSON.stringify({ status: next }) });
        await loadTasks();
  ...
  document.getElementById('task-add-btn').addEventListener('click', async () => {
    ...
    try {
      await api('/tasks/', { method: 'POST', body: JSON.stringify({
        title, subject: subject || null, due_date: due, priority, memo: memo || null,
      })});
  ...
  el.innerHTML = gradesData.map(g => {
    ...
    return `<div class="grade-item" data-id="${g.id}">
      <div class="grade-item-left">
        ...
        <div class="grade-meta">${g.date} ・ ${TASK_TYPE_LABELS[g.grade_type] || g.grade_type}${g.memo ? '・' + g.memo : ''}</div>
      </div>
...
function renderAttendance() {
  ...
  if (!attendanceData.length) { el.innerHTML = '<p class="db-empty">科目を登録してください</p>'; return; }
  el.innerHTML = attendanceData.map(a => {
    ...
    return `<div class="db-card" style="margin-bottom:10px;">
      ...
      <div style="display:flex;gap:8px;margin-top:10px;">
        ...
        <button class="btn-secondary att-edit-btn" data-id="${a.id}"
          data-subject="${a.subject}" data-total="${a.total_classes}"
          data-attended="${a.attended}" data-max="${a.max_absences}"
          style="padding:8px 12px;font-size:12px;">編集</button>
  ...
  document.querySelectorAll('.att-attend-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      try {
        // delta:1 = 出席（授業数+1, 出席数+1）
        await api(`/attendance/${btn.dataset.id}/attend`, { method: 'PATCH', body: JSON.stringify({ delta: 1 }) });
  ...
  document.querySelectorAll('.att-absent-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      try {
        // delta:-1 = 欠席（授業数+1, 出席数変化なし）← バグ修正: 旧 delta:0 → -1
        await api(`/attendance/${btn.dataset.id}/attend`, { method: 'PATCH', body: JSON.stringify({ delta: -1 }) });
        await loadAttendance();
      } catch(e) { toast(e.message || '失敗', 'error'); }
    });
  ...
  document.getElementById('att-save-btn').addEventListener('click', async () => {
    ...
    try {
      await api('/attendance/', { method: 'POST', body: JSON.stringify({
        subject, total_classes: total, attended, max_absences: max,
      })});
