...
// ============================================================
let _cropFile = null;
let _cropZoom = 1, _cropX = 50, _cropY = 50;
let _cropSrc  = null;

function openCrop(file) {
  _cropFile = file;
  _cropZoom = 1; _cropX = 50; _cropY = 50;
  const url = URL.createObjectURL(file);
  _cropSrc  = url;
...
function renderCrop() {
  ...
  const px     = parseFloat(document.getElementById('crop-x').value);
  const py     = parseFloat(document.getElementById('crop-y').value);
  const size   = 200 / zoom;
  const maxX   = img.naturalWidth  - size;
  const maxY   = img.naturalHeight - size;
  const ox     = (px / 100) * maxX;
  const oy     = (py / 100) * maxY;
  ...
  // crop-img の transform で位置・ズームをプレビュー
  const scaleW = 260 / img.naturalWidth;
  img.style.transform = `scale(${zoom}) translate(${-(px/100)*(1-1/zoom)*img.naturalWidth/2*scaleW/zoom}px, ${-(py/100)*(1-1/zoom)*img.naturalHeight/2*scaleW/zoom}px)`;
}
...
function applyCrop() {
  ...
  URL.revokeObjectURL(_cropSrc);
  return canvas.toDataURL('image/jpeg', 0.85);
}

document.getElementById('crop-zoom').addEventListener('input', renderCrop);
document.getElementById('crop-x').addEventListener('input', renderCrop);
document.getElementById('crop-y').addEventListener('input', renderCrop);
...
document.getElementById('crop-apply-btn').addEventListener('click', async () => {
  ...
  const msg = document.getElementById('avatar-msg');
  msg.style.color = '#7a87aa'; msg.textContent = 'アップロード中...';
  try {
    await api('/users/me/avatar', { method: 'PATCH', body: JSON.stringify({ avatar: dataUrl }) });
    showAvatar(dataUrl);
    msg.style.color = '#3ecf8e'; msg.textContent = 'アバターを更新しました';
  } catch(e) {
    msg.style.color = '#f0476c'; msg.textContent = e.message || 'アップロードに失敗しました';
  }
});
