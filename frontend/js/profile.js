...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('crop-save-btn').addEventListener('click', async () => {
    ...
    if (dataUrl.length > 2 * 1024 * 1024) {
      ...
      msg.textContent  = '画像が大きすぎます。ズームを下げてください';
      return;
    }
    document.getElementById('crop-modal').classList.add('hidden');
    ...
    try {
      ...
      msg.style.color = '#f0476c';
      msg.textContent = e.message || '保存に失敗しました';
    }
    document.getElementById('avatar-input').value = '';
  });
  ...
  document.getElementById('avatar-delete-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      msg.style.color = '#f0476c';
      msg.textContent = e.message || '削除に失敗しました';
    }
  });
  ...
  document.getElementById('change-password-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      msg.style.color = '#f0476c';
      msg.textContent = e.message || 'パスワード変更に失敗しました';
    }
  });
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (saveMsg) {
          ...
          saveMsg.textContent = 'プロフィールを保存しました';
        }
        ...
        if (saveMsg) {
          ...
          saveMsg.textContent = e.message || '保存に失敗しました';
        }
      }
    });
  }
...
function renderEquipGrid() {
  ...
  if (!invA.length && !invB.length) {
    list.innerHTML = '<p style="font-size:12px;color:var(--text-3);">まだ称号がありません。発掘ページでガチャを引いてください！</p>';
