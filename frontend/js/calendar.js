...
async function init() {
  ...
  const now = new Date();
  currentYear = now.getFullYear(); currentMonth = now.getMonth(); selectedDate = dateStr(now);
  await Promise.all([loadEvents(), loadXP()]);
  renderCalendar(); renderDayEvents(selectedDate); checkReminders();
...
function renderCalendar() {
  ...
  const eventMap = {};
  allEvents.forEach(e => { if (!eventMap[e.date]) eventMap[e.date] = []; eventMap[e.date].push(e); });
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (ds===today) cls+=' today'; if (ds===selectedDate) cls+=' selected';
    el.className = cls;
    const num = document.createElement('span');
    num.className = 'cal-date-num'; num.textContent = d; el.appendChild(num);
    if (eventMap[ds]) {
    ...
    el.addEventListener('click', () => { selectedDate = ds; renderCalendar(); renderDayEvents(ds); });
    grid.appendChild(el);
  }
  renderExamCountdown();
}
...
function renderDayEvents(ds) {
  ...
  if (!events.length) { list.innerHTML = '<p class="no-events">予定はありません</p>'; return; }
  ...
  list.querySelectorAll('.ev-done-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (!ev) return;
      try {
        const data = await api(`/calendar/${id}`, { method:'PATCH', body:JSON.stringify({ title:ev.title, memo:ev.memo, date:ev.date, type:ev.type, is_done:!ev.is_done }) });
        ...
        renderDayEvents(selectedDate); renderCalendar();
...
document.getElementById('ev-cancel-btn').addEventListener('click', () => { document.getElementById('event-modal').classList.add('hidden'); });
...
document.getElementById('ev-save-btn').addEventListener('click', async () => {
  ...
  try {
    if (editingEventId) {
      ...
      const data = await api(`/calendar/${editingEventId}`, { method:'PATCH', body:JSON.stringify({ title, memo, date, type, is_done:ev.is_done }) });
    ...
    renderCalendar(); renderDayEvents(selectedDate);
