...
function mult(atkType, defType) { return (CHART[atkType]||{})[defType] ?? 1; }
function multCombined(atkType, d1, d2) { let m = mult(atkType,d1); if (d2&&d2!==d1) m*=mult(atkType,d2); return m; }
...
function typeTag(t) { return `<span class="type-tag" style="background:${TYPE_COLOR[t]}">${t}</span>`; }
...
function buildAtkGrid() {
  ...
  TYPES.forEach(t => {
    ...
    btn.className = 'type-select-btn'; btn.textContent = t; btn.style.background = TYPE_COLOR[t];
    btn.addEventListener('click', () => {
      grid.querySelectorAll('.type-select-btn').forEach(b => b.classList.remove('selected'));
      btn.classList.add('selected'); showAtkResult(t);
    });
...
function showAtkResult(atkType) {
  ...
  TYPES.forEach(def => { const m = mult(atkType,def); if (groups[m]) groups[m].push(def); });
  let html = `<div class="result-title">${typeTag(atkType)} で攻撃したとき</div>`;
  [[4,'×4'],[2,'×2'],[0.5,'×½'],[0.25,'×¼'],[0,'×0 無効']].forEach(([m, label]) => {
    if (groups[m]?.length) {
      html += `<div class="result-row"><span class="mult-badge ${multLabel(m).cls}">${label}</span>${groups[m].map(typeTag).join('')}</div>`;
    }
  });
  box.innerHTML = html;
}
  ...
  TYPES.forEach(t => {
    ...
    btn.addEventListener('click', () => {
      if (selectedDefTypes.includes(t)) {
        selectedDefTypes = selectedDefTypes.filter(x => x!==t); btn.classList.remove('selected');
      } else {
        if (selectedDefTypes.length>=2) {
          const first = selectedDefTypes.shift();
          grid.querySelectorAll('.type-select-btn').forEach(b => { if (b.textContent===first) b.classList.remove('selected'); });
        }
        selectedDefTypes.push(t); btn.classList.add('selected');
      }
      showDefResult();
    });
    grid.appendChild(btn);
...
function showDefResult() {
  if (!selectedDefTypes.length) { document.getElementById('def-result').innerHTML=''; return; }
  const [def1, def2] = selectedDefTypes;
  const groups = {4:[],2:[],1:[],0.5:[],0.25:[],0:[]};
  TYPES.forEach(atk => { const m=multCombined(atk,def1,def2||''); if(groups[m]!==undefined) groups[m].push(atk); else groups[1].push(atk); });
  ...
  [[4,'×4'],[2,'×2 弱点'],[1,'×1'],[0.5,'×½ 耐性'],[0.25,'×¼ 耐性'],[0,'×0 無効']].forEach(([m, lbl]) => {
    if (groups[m]?.length) {
      html += `<div class="result-row"><span class="mult-badge ${multLabel(m).cls}">${lbl}</span>${groups[m].map(typeTag).join('')}</div>`;
    }
  });
  document.getElementById('def-result').innerHTML = html;
}

function buildFullTable() {
  const table = document.getElementById('type-full-table');
  let html = '<thead><tr><th class="tt-corner">攻撃↓ / 防御→</th>';
  TYPES.forEach(t => { html += `<th><span class="type-tag-v" style="background:${TYPE_COLOR[t]}">${t}</span></th>`; });
  html += '</tr></thead><tbody>';
  TYPES.forEach(atk => {
    html += `<tr><th><span class="type-tag" style="background:${TYPE_COLOR[atk]}">${atk}</span></th>`;
    TYPES.forEach(def => {
      const m = mult(atk,def); const { cls } = multLabel(m);
...
document.getElementById('calc-btn').addEventListener('click', () => {
  ...
  let typeEff = 1;
  if (moveType&&def1) typeEff *= mult(moveType,def1);
  if (moveType&&def2) typeEff *= mult(moveType,def2);
  ...
  const typeCls   = typeEff===0?'mult-0':typeEff>=2?'mult-2':typeEff<=0.5?'mult-05':'mult-1';
  const ko = damage>=hp ? '🎯 <b>確定1発！</b>' : damage*2>=hp ? '⚡ 確定2発' : damage*3>=hp ? '確定3発' : `${Math.ceil(hp/damage)}発`;

  document.getElementById('calc-result').innerHTML = `
    <div class="calc-result-main">
      <div class="calc-result-dmg">${damage} ダメージ</div>
      <div class="calc-result-pct">${pct}% (${(minDmg/hp*100).toFixed(1)}%〜${(maxDmg/hp*100).toFixed(1)}%)</div>
      <div class="calc-result-ko">${ko}</div>
    </div>
    <div class="calc-result-detail">
      <span class="mult-badge ${typeCls}">${typeLabel}</span>
...
function renderGachaInventory() {
  ...
  if (!invA.length&&!invB.length) {
    el.innerHTML = '<p style="color:var(--text-3);text-align:center;padding:16px;">まだ称号がありません</p>'; return;
  }
  const mkList = (inv, type, col) => [...inv].sort(sort).map(i =>
...
async function init() {
  ...
  buildAtkGrid(); buildDefGrid(); buildFullTable(); buildTypeSelects();
  await loadGachaXP();
  await fetchInventoryFromDB();
  renderGachaInventory();
}
