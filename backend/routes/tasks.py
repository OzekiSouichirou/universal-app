...
def add_task(body: TaskBody, db=Depends(get_db), current_user=Depends(get_current_user)):
    if not body.title.strip(): err(E.VALIDATION, "タイトルを入力してください")
    if body.priority not in ('high', 'medium', 'low'): err(E.VALIDATION, "優先度が不正です")
    row = db.execute(text("""
        INSERT INTO tasks (username,title,subject,due_date,priority,memo)
        VALUES (:u,:t,:s,:d,:p,:m) RETURNING id,title,subject,due_date,priority,status,memo
...
def update_task(task_id: int, body: TaskBody, db=Depends(get_db), current_user=Depends(get_current_user)):
    row = db.execute(text("""
        ...
        WHERE id=:id AND username=:u RETURNING id,title,subject,due_date,priority,status,memo
    """), {"id": task_id, "u": current_user.username, "t": body.title.strip(),
           "s": body.subject, "d": body.due_date, "p": body.priority, "m": body.memo}).fetchone()
    if not row: err(E.NOT_FOUND, "課題が見つかりません", 404)
    return ok(row_to_dict(row))

@router.patch("/{task_id}/status")
def update_status(task_id: int, body: StatusBody, db=Depends(get_db), current_user=Depends(get_current_user)):
    if body.status not in ('pending', 'in_progress', 'done'): err(E.VALIDATION, "ステータスが不正です")
    row = db.execute(text("""
        UPDATE tasks SET status=:st WHERE id=:id AND username=:u
...
def delete_task(task_id: int, db=Depends(get_db), current_user=Depends(get_current_user)):
    db.execute(text("DELETE FROM tasks WHERE id=:id AND username=:u"),
